...
export default async function CostBevPage() {
  ...
  const [{ data: products }, { data: stock }, { data: bevSales }] = await Promise.all([
    supabase
      .from('products')
    ...
    supabase
      ...
      .in('menu_item.outlet', ['lunch', 'dinner', 'room_service', 'banqueting_lunch', 'banqueting_dinner'])
  ...
  const totalRevenue = (bevSales ?? []).reduce((sum, s) => {
    const price = (s.menu_item as { price_excl_tax?: number } | null)?.price_excl_tax ?? 0
    return sum + price * s.quantity
  }, 0)

  const bevCostRatio = totalRevenue > 0 ? (totalPurchased / totalRevenue) * 100 : 0

  return (
    <div className="space-y-6 w-full max-w-5xl">
      <div>
        <h1 className="text-xl font-bold text-[#3D1640]">Cost Bev</h1>
        <p className="text-sm text-[#B0A5B4] mt-0.5">{monthLabel(month)} — Ratio coût matières boissons</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <Card>
          <CardContent className="py-6 text-center">
            ...
            <p className="text-xs text-[#B0A5B4] mt-1 uppercase tracking-wide">Achats boissons HT</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="py-6 text-center">
            <p className="text-2xl font-bold text-green-700">{formatCurrency(totalRevenue)}</p>
            <p className="text-xs text-[#B0A5B4] mt-1 uppercase tracking-wide">CA Boissons HT</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="py-6 text-center">
            <p className={`text-2xl font-bold ${bevCostRatio > 30 ? 'text-red-600' : bevCostRatio > 22 ? 'text-amber-600' : 'text-green-700'}`}>
              {bevCostRatio.toFixed(1)}%
            </p>
            <p className="text-xs text-[#B0A5B4] mt-1 uppercase tracking-wide">Bev Cost Ratio</p>
          </CardContent>
        </Card>
      </div>

      <Card>
        <CardHeader><CardTitle>Stock boissons — {monthLabel(month)}</CardTitle></CardHeader>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-[#F9F7F4] border-b border-[#E5E2D8]">
                ...
                <th className="px-4 py-2 text-right text-xs font-semibold text-[#7B6B80] uppercase tracking-wider">Coût consommé</th>
              </tr>
            </thead>
            <tbody>
              {(products ?? []).length === 0 ? (
                <tr><td colSpan={5} className="px-4 py-8 text-center text-[#B0A5B4]">Aucun produit boisson.</td></tr>
              ) : (products ?? []).map(p => {
                const s = stock?.find(s => s.product_id === p.id)
                ...
                return (
                  <tr key={p.id} className="border-b border-[#E5E2D8] hover:bg-[#FAFAF8]">
                    ...
                    <td className="px-4 py-3 text-right font-mono text-green-700">{s?.bought ?? 0}</td>
                    <td className="px-4 py-3 text-right font-mono text-[#7B6B80]">{usedQty}</td>
